...
namespace LibraryEnergizingPowerLine
{
    public class EfficiencyMeansControlVoltage
    {
        ...
        /// <param name="switchMCV">Номер СРН.</param>
            ...
            int switchMCV)
        {
            ...
            // Обращение к колонке Реактивная проводимость реактора.
            ICol BReactor = (ICol)tableReactor.Cols.Item("B");
            ...
            {
                if ((double)BReactor.Z[indexReactor] > 0)
                {
                ...
                else if ((double)BReactor.Z[indexReactor] < 0)
                {
                    if ((bool)staReactor.Z[indexReactor] == true)
                    {
                        return $"БСК <<{nameR}>> уже отключен";
                    }

                    else if ((bool)staReactor.Z[indexReactor] == false)
                    {
                        staReactor.Z[indexReactor] = 1;
                        indexReactor = tableReactor.FindNextSel[indexReactor];
                    }
                }
                break;
